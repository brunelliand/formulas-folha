...
def evento = [codigo: 168 , taxa: 0 , tipo: TipoEvento.VENCIMENTO , unidade: UnidadeEvento.AUTOMATICO];
...
if ( vaux <= 0 ){
    if ( ( TipoProcessamento.FERIAS.equals(calculo.tipoProcessamento) && folha.diasGozo ) || ( mensalIntegral ) ){
        vaux = BasesOutrasEmpresas.buscaPor(TipoProcessamento.MENSAL).sum(0, { it.baseInss })
    }else{
