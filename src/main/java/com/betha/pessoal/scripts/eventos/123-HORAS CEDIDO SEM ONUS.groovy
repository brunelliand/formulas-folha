...
def evento = [codigo: 123, taxa: 0, tipo: TipoEvento.VENCIMENTO, unidade: UnidadeEvento.AUTOMATICO];
...
if (vaux > 0) {
    ...
    vaux = Funcoes.cnvdpbase(Funcoes.cedidosemonus())
    if (vaux < 0) {
        vaux = 0
    }
    valorReferencia = vaux
...
if (valorCalculado > 0) {
    if (Eventos.valor(1) == 0) {
        Bases.compor(remuneracao, Bases.HORAEXTRA, Bases.SIND)
    }
