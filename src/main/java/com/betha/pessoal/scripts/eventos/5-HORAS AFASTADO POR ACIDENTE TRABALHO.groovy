...
def evento = [codigo: 5, taxa: 0, tipo: TipoEvento.VENCIMENTO, unidade: UnidadeEvento.AUTOMATICO];
...
if (vaux >= 0) {
    ...
    if ((Funcoes.afasacidtrab() == 0)) {
        suspender "Cálculo do evento cancelado"
    }
    vaux = Funcoes.cnvdpbase(Funcoes.afasacidtrab())
    valorReferencia = vaux
}
