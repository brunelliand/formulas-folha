...
def evento = [codigo: 901, taxa: 0, tipo: TipoEvento.VENCIMENTO, unidade: UnidadeEvento.AUTOMATICO];

/** INICIO DA FÓRMULA DO EVENTO DE CÁLCULO DA FOLHA **/

if (!calculo.tipoProcessamento.equals(TipoProcessamento.MENSAL)) {
    suspender "Calcular somente no processamento mensal"
};
double provento = 0;
double desconto = 0;
...
desconto = desconto.abs();
if (desconto > 0) {
    valorCalculado = desconto;
}
